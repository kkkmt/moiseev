...
CREATE TABLE IF NOT EXISTS `kindergarten`.`tbl_payslipPeople` (
  ...
  `PersonnelNumber` INT UNSIGNED NOT NULL,
  `Name` VARCHAR(80) NOT NULL,
  `Post` VARCHAR(60) NOT NULL,
  `Salary` DOUBLE UNSIGNED NOT NULL,
  `WorkedDays` INT UNSIGNED NOT NULL,
  `WorkedDaysWeekends` INT UNSIGNED NULL,
  `Credited` DOUBLE UNSIGNED NOT NULL,
  `CreditedAll` DOUBLE UNSIGNED NOT NULL,
  `Tax` DOUBLE UNSIGNED NULL,
  `Other` DOUBLE UNSIGNED NULL,
  `WithheldAll` DOUBLE UNSIGNED NULL,
  `DutyOrganization` DOUBLE UNSIGNED NULL,
  `DutyWorker` DOUBLE UNSIGNED NULL,
  `ResultSum` DOUBLE NOT NULL,
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `kindergarten`.`GetChildren` (`ID` INT, `FName` INT, `LName` INT, `PName` INT, `Birth` INT, `Address` INT, `Group` INT, `MFName` INT, `MLName` INT, `MPName` INT, `MPhone` INT, `FFName` INT, `FLName` INT, `FPName` INT, `FPhone` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `kindergarten`.`GetPersonnel` (`ID` INT, `FName` INT, `LName` INT, `PName` INT, `Post` INT, `Salary` INT, `DateReceipt` INT, `DateDismissal` INT);
...
DELIMITER $$
USE `kindergarten`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdatePost`(IN ID INT UNSIGNED, IN Post VARCHAR(45))
BEGIN
	UPDATE tbl_personnel SET tbl_personnel.Post = Post
	WHERE tbl_personnel.ID = ID;
END$$
...
DELIMITER $$
USE `kindergarten`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateSalary`(IN ID INT UNSIGNED, IN Salary DOUBLE)
BEGIN
	UPDATE tbl_personnel SET tbl_personnel.Salary = Salary
	WHERE tbl_personnel.ID = ID;
END$$
...
DELIMITER $$
USE `kindergarten`$$
CREATE DEFINER=`root`@`localhost` FUNCTION `AddEmployee`(FName VARCHAR(45), LName VARCHAR(45), PName VARCHAR(45), Post VARCHAR(45), Salary DOUBLE) 
RETURNS DATE
BEGIN
	DECLARE Now DATE;
	SET Now = NOW();
    
	INSERT INTO tbl_personnel (`FName`, `LName`, `PName`, `Post`, `Salary`, `DateReceipt`) VALUES (FName, LName, PName, Post, Salary, Now);
...
-- -----------------------------------------------------
-- function DismissalEmployee
-- -----------------------------------------------------

DELIMITER $$
USE `kindergarten`$$
CREATE DEFINER=`root`@`localhost` FUNCTION `DismissalEmployee`(ID INT UNSIGNED) 
RETURNS DATE
BEGIN
	DECLARE Now DATE;
	SET Now = NOW();
    
	UPDATE tbl_personnel SET tbl_personnel.DateDismissal = Now
	WHERE tbl_personnel.ID = ID;
...
GROUP BY tbl_personnel.LName, tbl_personnel.FName, tbl_personnel.PName;
